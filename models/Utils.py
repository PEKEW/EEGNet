...
class CommonExtraction:
    ...
    def _get_common_base(self):
        for _ in range(self.max_iter):
            ...
            denominator = np.dot(self.W, np.dot(
                self.H1, self.H1.T) + np.dot(self.H2, self.H2.T))
            ...
            H1_diff = np.linalg.norm(self.H1 - H1_old) / \
                np.linalg.norm(self.H1)
            H2_diff = np.linalg.norm(self.H2 - H2_old) / \
                np.linalg.norm(self.H2)
        ...
        for pattern in self.patten_analysis:
            if pattern['pattern_type'] == 'Common':
                H_common[pattern['pattern_idx'], :] = (
                    pattern['G1_activation'] + pattern['G2_activation']) / 2
